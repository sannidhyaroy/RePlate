<template>
  <div class="page-container m-4 py-6 px-10 text-white min-h-screen">
    <!-- Leave space for Navbar -->
    
    <h1 class="font-bold text-5xl mt-4 mb-2">Profile</h1>
    

    <!-- <h1 class="font-bold text-5xl mb-2">Profile</h1> -->
    <USeparator class="my-2" color="primary" />

      <!-- Profile Section -->
    
    <div class="flex flex-col md:flex-row items-center md:items-start gap-6 mt-6 px-4 md:px-10">
      <!-- Profile Picture -->
      <div class="w-32 h-32 border-4 border-gray-300 rounded-full flex items-center justify-center text-sm text-gray-400 shrink-0">
      ...
      <div class="flex flex-col gap-2 justify-center md:justify-start">
          ...
          <UButton variant="soft"
                  class="bg-red-500 hover:bg-red-900 text-white" @click="changeDetails">Change Details</UButton>
          <UButton variant="soft"
                    class="bg-red-500 hover:bg-red-900 text-white" @click="deleteAccount">Delete Account</UButton>
          </div>
      </div>
    </div>
    ...
    <div class="mt-10">
      ...
      <div class="grid grid-cols-2 sm:grid-cols-4 gap-4">
        <UCard
          v-for="(item, index) in cookingHistory"
          :key="index"
    ...
    <div class="mt-10">
      ...
      <USeparator class="mb-4" color="primary" />
      <UCard class="p-4">
        <textarea v-model="comment" class="w-full p-2 bg-transparent border border-green-400 rounded-md text-white" rows="4" placeholder="Write your comment..."></textarea>
        <div class="flex justify-center mt-4">
          <UButton leading-icon="i-lucide-upload" class="mr-2" color="primary" @click="submitComment">SUBMIT COMMENT</UButton>
        </div>
      </UCard>
    </div>
  </div>
